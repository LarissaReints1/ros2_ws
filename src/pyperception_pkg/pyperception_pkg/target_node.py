...
class RedTargetDepthNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(f"Loading ZoeDepth on {self.device}...")

        # Load ZoeDepth metric variant
        self.model = torch.hub.load('isl-org/ZoeDepth', 'ZoeD_NK', pretrained=True)
        self.model.to(self.device)
        self.model.eval()

        # Standard preprocessing for ZoeDepth
        ...
        self.depth_map = None
        # Run depth at lower rate (5 Hz)
        self.depth_timer = self.create_timer(0.2, self.depth_loop)

        self.get_logger().info("ZoeDepth loaded successfully.")
    ...
    def depth_loop(self):
        ...
        img_rgb = cv2.cvtColor(small, cv2.COLOR_BGR2RGB) / 255.0
        input_tensor = self.transform(img_rgb).unsqueeze(0).to(self.device)

        # ---------------- Inference ----------------
        with torch.no_grad():
            depth_pred = self.model(input_tensor)
            depth_pred = depth_pred.squeeze().cpu().numpy()  # approximate meters

        # ---------------- Upscale to original resolution ----------------
        depth = cv2.resize(depth_pred, (w, h))
    ...
    def image_callback(self, msg: Image):
        ...
        self.latest_frame = frame  # store for depth thread
        ...
        point.point.z = float(depth if u != -1 else -1)
